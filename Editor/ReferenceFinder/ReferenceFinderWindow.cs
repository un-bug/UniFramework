...
{
    private const string ToolName = "Reference Finder";
    //依赖模式的key
    ...
    //查找资源引用信息
    [MenuItem("Assets/" + ToolName + " %#&f", false, 25)]
    static void FindRef()
    {
        InitDataIfNeeded();
        OpenWindow();
    ...
    //打开窗口
    [MenuItem("Window/" + ToolName, false, 1000)]
    ...
    public void DrawOptionBar()
    {
        ...
        isDepend = GUILayout.Toggle(isDepend, isDepend ? "Dependencies" : "References", toolbarButtonGUIStyle,GUILayout.Width(110));
    ...
    private  AssetViewItem CreateTree(string guid, ref int elementCount, int _depth, Stack<string> stack)
    {
        ...
        ++elementCount;
        ReferenceFinderData.AssetDescription referenceData;
        if (!data.assetDict.TryGetValue(guid, out referenceData))
            return null;
